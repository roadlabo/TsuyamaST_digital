...
以下が正しかったため、一部動作していた可能性が高い：
・manualpeerlist の指定  
・resync コマンドの実行  

一方で、以下が不足していた可能性がある：
・親機が reliable に設定されていなかった  
・NtpServer が有効になり切っていなかった  

結果として、Windows 内部判定で「親として失格」になっていた。
...
・子機同士で同期させる  
・親機を複数構成する  
・resync を高頻度で実行する（1日1回で十分）

---

### 設計方針まとめ（最終記述文）

本システムでは Windows Time Service（w32tm）を用い、
AI解析用PCをLAN内の基準時刻（親機）とする。
サイネージPCは当該親機を参照先として設定し、
定期的に時刻同期を試行する。
親機が利用不可の場合は自己時刻で動作を継続し、
復旧後に自動的に再同期する。
本構成は閉鎖ネットワーク・無人運用・端末増設に適した
堅牢な時刻同期方式である。
