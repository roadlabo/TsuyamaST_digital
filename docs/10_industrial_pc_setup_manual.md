...
### 目的
無人運用のサイネージPCでは、常駐アプリ `pc_agent.py` が何らかのエラーで停止した場合でも、自動で復旧できるようにする仕組みを導入する。  
これにより、短時間の障害や突発的な例外発生後でも継続稼働を確保する。
...
このバッチは、`pc_agent.py` を監視し、停止した場合は再起動、  
一定時間内に複数回停止した場合は OS を再起動する機能を持つ。
...
同一時間枠内（例：10 分）に連続回数以上（例：3 回）停止した場合：  
OS の再起動を行う（ハードウェア依存問題等を想定）。

ログ出力先：
- `C:\_TsuyamaSignage\logs\pc_agent_watchdog.log`
- `C:\_TsuyamaSignage\logs\pc_agent_crash_state.csv`

ログローテーションや肥大化対策がバッチ内で自動で行われる。

### 11-6 まとめ
この設定を導入することで、次のような運用課題を解決できる。

- アプリの予期しない停止 → 自動復旧
- 短時間での異常停止 → OS 自動再起動

### 11-7 補足（運用上の注意）
- `pc_agent.py` のパスを変更した場合は、watchdogバッチ側のパスも合わせて修正すること。
- バッチとタスクスケジューラは **管理者権限で設定すること**（サービス的に永続実行するため）。
- ログは定期的に収集し、GitHub には含めない（`.gitignore` へ登録済み）。
---
